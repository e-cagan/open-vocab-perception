...
def main(
    ...
    # Build components
    det_instance = DETECTOR_REGISTRY.create(
        detector, device=device, threshold=threshold, dtype=dtype
    )
    ...
    with VideoReader(video) as reader:
        console.print(
            f"[cyan]Video:[/cyan] {reader.width}x{reader.height} @ {reader.fps:.1f} FPS, {reader.frame_count} frames"
        )

        n_frames_to_process = (
            min(reader.frame_count, max_frames) if max_frames > 0 else reader.frame_count
        )

        with VideoWriter(
            output, fps=reader.fps, width=reader.width, height=reader.height
        ) as writer:
